...
import {
  ArrowLeft, Layers, Paperclip, Activity, CheckCircle,
  RotateCcw, Upload, Download, Trash2, AlertTriangle, User,
  Building2, Calendar, Hash, Tag
} from 'lucide-react';
...
export default function ShopDrawingDetailPage() {
  const { id } = useParams();
  ...
  const fetch_ = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/shop-drawings/${id}`, { headers });
      if (!res.ok) throw new Error('Not found');
      setDrawing(await res.json());
    } catch {
      router.push('/dashboard/manager/shop-drawings');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/shop-drawings/${id}/status`, {
        method: 'PATCH',
      ...
      if (data.newId) {
        router.push(`/dashboard/manager/shop-drawings/${data.newId}`);
      } else {
  ...
  const handleAttachmentUpload = async (e) => {
    ...
    try {
      await fetch(`${API_BASE}/api/shop-drawings/${id}/attachments`, {
        method: 'POST',
  ...
  const handleDeleteAttachment = async (attId) => {
    ...
    await fetch(`${API_BASE}/api/shop-drawings/${id}/attachments/${attId}`, { method: 'DELETE', headers });
    fetch_();
  };
  ...
  const cfg = STATUS_CONFIG[drawing.status] || { label: drawing.status, cls: 'badge badge-gray', next: [] };
  const discColor = DISCIPLINE_COLORS[drawing.discipline] || '#9CA3AF';
      ...
      <div className="p-6 space-y-6 max-w-5xl mx-auto">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <span
                  ...
                  style={{ background: discColor + '22', color: discColor, border: `1px solid ${discColor}` }}
                >
                  {drawing.discipline}
                </span>
              </div>
              <p className="text-gray-500">{drawing.title}</p>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-400">Rev. {drawing.revisionLetter || 'A'} / {drawing.revisionNumber || 0}</span>
        ...
        <div className="border-b border-gray-200">
          <nav className="flex gap-6">
            {['overview', 'attachments', 'activity'].map((t) => (
              ...
              >
                {t === 'attachments' ? `Attachments (${drawing.attachments?.length || 0})` : t}
              </button>
            ))}
          </nav>
        </div>
        ...
        {tab === 'overview' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="card card-body space-y-4">
              <h3 className="font-semibold text-gray-800">Drawing Details</h3>
              <Field icon={<Hash size={15} />}      label="Drawing Number"  value={drawing.drawingNumber} />
              <Field icon={<Tag size={15} />}        label="Title"           value={drawing.title} />
              <Field icon={<Tag size={15} />}        label="Discipline"      value={drawing.discipline} />
              <Field icon={<Building2 size={15} />}  label="Project"         value={drawing.projectName || '—'} />
              <Field icon={<User size={15} />}       label="Vendor"          value={drawing.vendor?.companyName || '—'} />
              <Field icon={<User size={15} />}       label="Reviewer"        value={drawing.reviewer?.name || '—'} />
            </div>
            <div className="card card-body space-y-4">
              <h3 className="font-semibold text-gray-800">Revision & Timeline</h3>
              <Field icon={<RotateCcw size={15} />} label="Revision Letter"  value={drawing.revisionLetter || 'A'} />
              <Field icon={<RotateCcw size={15} />} label="Revision Number"  value={String(drawing.revisionNumber || 0)} />
              <Field icon={<Calendar size={15} />}  label="Submission Date"  value={drawing.submissionDate ? new Date(drawing.submissionDate).toLocaleDateString() : '—'} />
              <Field icon={<Calendar size={15} />}  label="Required Date"    value={drawing.requiredDate ? new Date(drawing.requiredDate).toLocaleDateString() : '—'} />
              <Field icon={<Calendar size={15} />}  label="Created"          value={new Date(drawing.createdAt).toLocaleDateString()} />
              {drawing.delayDays > 0 && (
                <div className="flex items-center gap-2 p-3 bg-red-50 rounded-lg border border-red-100">
                  <AlertTriangle size={16} className="text-red-500 flex-shrink-0" />
                  <span className="text-sm text-red-700 font-medium">{drawing.delayDays} days overdue</span>
                </div>
              )}
            </div>
            {drawing.notes && (
              <div className="card card-body md:col-span-2">
                ...
                <p className="text-sm text-gray-600 whitespace-pre-wrap">{drawing.notes}</p>
              </div>
            )}
          </div>
        )}
        ...
        {tab === 'attachments' && (
          <div className="space-y-4">
            ...
            {(!drawing.attachments || drawing.attachments.length === 0) ? (
        ...
        {tab === 'activity' && (
          <div className="space-y-3">
            {(!drawing.activityLog || drawing.activityLog.length === 0) ? (
              <div className="card card-body text-center py-12 text-gray-400">
