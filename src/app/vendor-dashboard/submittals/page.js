...
import { FileText, AlertTriangle, CheckCircle, Clock, RotateCcw, Search, XCircle, Eye, Calendar, ChevronRight } from 'lucide-react';
...
export default function VendorSubmittalsPage() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/submittals?${params}`, {
  ...
  return (
    <ResponsiveLayout>
      <div className="p-6 space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">My Submittals</h1>
          <p className="text-sm text-gray-500 mt-0.5">Track all your material submittals</p>
        </div>
        ...
        {/* Resubmit Required alert */}
        {(counts.RESUBMIT_REQUIRED || 0) > 0 && (
          <div className="flex items-center gap-3 p-4 bg-orange-50 border border-orange-200 rounded-xl">
            <AlertTriangle size={20} className="text-orange-500 flex-shrink-0" />
            <div>
              <p className="text-sm font-medium text-orange-800">
                {counts.RESUBMIT_REQUIRED} submittal{counts.RESUBMIT_REQUIRED > 1 ? 's' : ''} require resubmission
              </p>
              <p className="text-xs text-orange-600">Review the feedback and resubmit your materials.</p>
            </div>
          </div>
        )}
        ...
        <div className="card">
          <div className="card-header flex items-center justify-between gap-3 flex-wrap">
            <h3 className="font-semibold text-gray-800">All Submittals <span className="text-sm text-gray-400 font-normal">({total})</span></h3>
            ...
            <table className="data-table">
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={7} className="text-center py-10 text-gray-400">No submittals found</td></tr>
                ) : list.map((s) => (
                  <tr key={s.id} className="cursor-pointer" onClick={() => router.push(`/dashboard/manager/material-submittals/${s.id}`)}>
            ...
            {loading ? (
              ...
              >
                ...
                <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-500">
                  ...
                  {s.delayDays > 0 && <span className="badge badge-red">{s.delayDays}d late</span>}
                </div>
              </div>
            ))}
          </div>
